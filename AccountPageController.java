...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String Gain_Momentum = "/my-account/gain-momentum-programme-page";
	...
	//Gain Momentum Slot
	@GetMapping("/gain-momentum-programme-page")
	@RequireHardLogIn
	public String getGainMomentum(final Model model) throws CMSItemNotFoundException
	{
		//Only Milk Supply & Trading Account users can have Bonuses
		final VetCustomerModel user = (VetCustomerModel) sessionService.getAttribute("user");

		if (StringUtils.isNotEmpty(user.getMilkSupplierId()) || StringUtils.isNotEmpty(user.getTradingAccountId()))
		{
			model.addAttribute(BREADCRUMBS, accountBreadcrumbBuilder.getBreadcrumbs("account.trading.gainMomentum.breadcrumbs"));
			storeCmsPageInModel(model, getContentPageForLabelOrId(Gain_Momentum));
			setUpMetaDataForContentPage(model, getContentPageForLabelOrId(Gain_Momentum));

			return ControllerConstants.Views.Pages.Account.GAIN_DASHBOARDPAGE;
		}
		//For non-milk and non-trading users,
		return REDIRECT_MY_ACCOUNT;
